        ...
        while not pq.is_empty():
            ...
            for (v, d, w) in successors:
                if v not in parents:
                    parents[v] = cur
                    costs[v] = costs[cur] + w
                    priority = costs[v] + self.heuristic(v, g.get_food())
                    pq.push(v, priority)
...
expanded, direciton, path = multibfs.search(multi_pacman, multi_pacman.get_start_state(), multi_pacman.get_goal_states())
